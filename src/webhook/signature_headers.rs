...
impl<'r> FromRequest<'r> for SignatureHeaders<'r> {
  ...
  async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
    ...
    if sig.is_none() {
      return Outcome::Error((Status::Unauthorized, SignatureHeaderError::MissingSignature))
    }

    let ts = request.headers().get_one("X-Signature-Timestamp");
    if ts.is_none() {
      return Outcome::Error((Status::Unauthorized, SignatureHeaderError::MissingTimestamp))
    }
